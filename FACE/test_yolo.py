...
def load_logged_names_from_excel():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row and row[0]:
                logged_names.add(str(row[0]).strip())
...
def mark_attendance(name: str, evidence_frame: Optional[np.ndarray] = None) -> bool:
    if name in INVALID_NAMES or name in logged_names:
        return False
...
# Hỗ trợ:
# known_faces/Nguyen Van A.jpg
# known_faces/Nguyen Van A/1.jpg
# ============================================================
    ...
    for person_name, image_path in all_items:
        try:
            ...
            if face_crop is None:
                print(f"  - Bỏ ảnh mẫu không hợp lệ (cần đúng 1 mặt): {image_path}")
                continue

            h, w = face_crop.shape[:2]
            if h < MIN_FACE_SIZE or w < MIN_FACE_SIZE:
                print(f"  - Ảnh mẫu quá nhỏ: {image_path}")
                continue

            if compute_blur_score(face_crop) < 20.0:
                print(f"  - Ảnh mẫu quá mờ, bỏ qua: {image_path}")
                continue

            embedding = build_face_embedding(face_crop)
            if embedding is None:
                print(f"  - Không tạo được embedding: {image_path}")
                continue
...
def recognize_face(face_crop: np.ndarray, face_w: int) -> Tuple[str, float]:
    ...
    try:
        lab = cv2.cvtColor(working_crop, cv2.COLOR_BGR2LAB)
        l, a, b = cv2.split(lab)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        cl = clahe.apply(l)
        limg = cv2.merge((cl, a, b))
        working_crop = cv2.cvtColor(limg, cv2.COLOR_LAB2BGR)
    except Exception:
        pass
